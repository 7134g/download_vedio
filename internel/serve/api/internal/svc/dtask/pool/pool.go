...
// 需要执行的任务
type Cell struct {
	CellFunc func([]interface{})
	Param    []interface{}
}

// 工人
type worker struct {
	cell          chan *Cell //工人工作任务，同一时间只能做一个
...
// 提交一个任务
func (p *Pool) Submit(cell *Cell) error {
	//没有厂房或厂房已关闭，那么报错
	if p == nil {
	...
	if p.Running() > p.Cap() { //如果超过上限，则清理掉一部分已完成任务的worker
		for !p.closed {
			...
			for i, w := range p.workers {
				//如果是等待任务的状态，可以清理
				if w.currentStatus() == Waiting {
					close(w.cell) //从队列中移除前必须先close chan
					p.workers = append(p.workers[:i], p.workers[i+1:]...)
					break
				}
			}
